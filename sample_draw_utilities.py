...
def draw_messiers_from_cartesian(screen, messiers, camera, color=(255,180,80)):
    ...
    Args:
        screen: Pygame screen surface
        messiers: List of (m_num, name, x, y, z, magnitude, type) tuples
        camera: Camera object from tracker.py
        color: Base color for Messier objects
    """
    for m_num, name, x, y, z, mag, obj_type in messiers:
        # Transform to camera space
        vc = camera.world_to_camera((x, y, z))
        ...
        # Draw Messier object as a distinctive marker
...
def draw_messiers(screen, messiers, yaw, pitch, fov, color=(255,180,80)):
    """Legacy function - use draw_messiers_from_cartesian instead."""
    width, height = screen.get_size()
    for mnum, name, ra, dec, t in messiers:
        p = spherical_to_cartesian(ra, dec)
        p = rotate_point(p, yaw, pitch)
        proj = project_point(p, fov, width, height)
        if proj:
            pygame.draw.circle(screen, color, proj, 3)
            pygame.draw.circle(screen, (255,255,0), proj, 4, 1)
